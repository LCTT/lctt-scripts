...
+ 1_add_new_article_newspaper.sh  :: 接口跟 =1_add_new_article_manual.sh= 类似但方便很多，它会尝试从页面中解析文章标题，文章日期，作者和主要内容。需要python3(并安装有newspaper库),jq,html2text(未来可能会改成pandoc)的支持
